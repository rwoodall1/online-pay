 ...
 Page.Response.AppendHeader("Cache-Control", "no-store, no-cache, must-revalidate, post-check=0, pre-check=0")
        ...
        If Not IsPostBack Then
            ...
            If ShopCart.basiconly = True Then
                ...
                Else
                    luvline.Visible = False
                End If
                If ShopCart.AdLine = True Then
                    If ShopCart.Adcuto < Date.Today Then
                        Ad.Visible = False
                    Else
                        SetAdoptions()
            ...
            Else 'or personalized
                ...
                If ShopCart.inkpers Then
                    rbBookType.Items.Add("Personalized Ink Yearbook")
                ElseIf ShopCart.inktxt Then
                    rbBookType.Items.Add("Personalized Ink Text")
                ElseIf ShopCart.foilpers Then
                    rbBookType.Items.Add("Personalized Foil Yearbook")
                ElseIf ShopCart.foiltxt Then
                    rbBookType.Items.Add("Personalized Foil Text")
                ElseIf ShopCart.picpers Then
                    rbBookType.Items.Add("Personalized Picture Yearbook")
                End If
                If ShopCart.LuvLine = True Then
                    luvline.Visible = True
                Else
    ...
    Protected Sub btnad_Click(sender As Object, e As EventArgs) Handles btnad.Click

        If rbadtype.SelectedValue <> "No Ad" Then
            If Page.IsValid Then
                If CheckForOrder() Then 'true is duplicate an order was found ask customer if it is a duplicate
                    Dim a As String = Session("duporder")
                    MsgBox1.Show("Possible Duplicate", a, Nothing, New EO.Web.MsgBoxButton("Yes", "", "yessubmit"), New EO.Web.MsgBoxButton("No", "", "nosubmit"))
    ...
    End Sub
    Protected Sub InsertOrder(pitem As String)
        Dim ShopCart As Cart = Session("Cart")
        ...
        If pitem = "yearbook" Then
            qty = txtqty.Text
            Select Case rbBookType.SelectedValue
                Case "Standard Yearbook"
                    itemamount = ShopCart.basicinvamt
                    item = "Standard Yearbook"
                Case "Personalized Ink Yearbook"
                    itemamount = ShopCart.inkpersamt
                    item = "Personalized Ink Yearbook"
                    mi1 = ddlicon1.SelectedValue
                    mi2 = ddlicon2.SelectedValue
                    mi3 = ddlicon3.SelectedValue
                    mi4 = ddlicon4.SelectedValue
                    Try
                ...
                Case "Personalized Foil Yearbook"
                    ...
                    item = "Personalized Foil Yearbook"
                    mi1 = ddlicon1.SelectedValue
                    mi2 = ddlicon2.SelectedValue
                    mi3 = ddlicon3.SelectedValue
                    mi4 = ddlicon4.SelectedValue
                ...
                Case "Personalized Picture Yearbook"
                    ...
                    item = "Personalized Picture Yearbook"

                Case "Personalized Foil Text"
                    itemamount = ShopCart.foiltxtamt
                    item = "Personalized Foil Text"

                Case "Personalized Ink Text"
                    itemamount = ShopCart.inktxtamt
                    item = "Personalized Ink Text"
            End Select

            item = rbBookType.SelectedValue
        ElseIf pitem = "luvline" Then
            itemamount = ShopCart.LuvLineAmt
            item = "Love Line"
        ElseIf pitem = "ad" Then
            Select Case rbadtype.SelectedValue
                Case "Full Page Ad"

                    itemamount = ShopCart.FullAdLineAmt
                Case "1/2 Page Ad"
                    itemamount = ShopCart.HalfAdLineAmt
                Case "1/4 Page Ad"
                    itemamount = ShopCart.QuarterAdLineAmt
                Case "1/8 Page Ad"
                    itemamount = ShopCart.EightAdLineAmt
            End Select
            item = rbadtype.SelectedValue
        End If
        ...
        dsorders.InsertParameters.Add("@itemqty", qty)
        dsorders.InsertParameters.Add("@itemtotal", (CInt(qty) * CDec(itemamount)))
        ...
        dsorders.InsertParameters.Add("@icon1", mi1)
        dsorders.InsertParameters.Add("@icon2", mi2)
        dsorders.InsertParameters.Add("@icon3", mi3)
        dsorders.InsertParameters.Add("@icon4", mi4)
...
cmd.connection.close()
        End Try
	End Sub
    Protected Sub TextBox1_TextChanged(sender As Object, e As System.EventArgs) Handles txtqty.TextChanged
        'Try
        '    If CInt(txtqty.Text) < 1 Then
        '        txtqty.Text = 1
        '    End If
        '    If rbBookType.SelectedValue = "LUVLINE" Then
        '        txtqty.Text = 1
        '    End If
        'Catch ex As Exception
        '    txtqty.Text = 1
...
cmd.Connection.Close()
    ...
    Protected Sub rbBookType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles rbBookType.SelectedIndexChanged
        Select Case rbBookType.SelectedValue
            Case "Standard Yearbook"
                txtqty.Enabled = True
                txtqty.Text = "1"
                ...
                End Try
            Case "Personalized Ink Text", "Personalized Foil Text"
            ...
            Case "Personalized Picture Yearbook"
                ...
                lblperpic.Visible = True
                lblperpic.Text = "(Student portrait and name will appear on the Cover)"
                txtqty.Enabled = True
                txtqty.Text = "1"
                txtperstext1.Text = ""
    ...
    'Protected Sub CustomValidator1_ServerValidate(source As Object, args As ServerValidateEventArgs) Handles valquantity.ServerValidate
    '    Dim qt As Integer = CInt(txtqty.Text)
    '    Dim retval As Boolean = True
    '    If rbBookType.SelectedValue = "LUVLINE" Then
    '        If qt > 1 Then
    '            retval = False
    '            valquantity.ErrorMessage = "LuvLine Quantity Can Only Be One For Each Quote"
    '        End If
    '    Else
    '        If qt = Nothing Then
    '            retval = False
    ...
    Protected Sub rbadtype_SelectedIndexChanged(sender As Object, e As EventArgs) Handles rbadtype.SelectedIndexChanged
